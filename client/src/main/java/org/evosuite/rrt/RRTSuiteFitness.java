...
public class RRTSuiteFitness extends TestSuiteFitnessFunction {
    ...
    private final static Logger logger = LoggerFactory.getLogger(RRTSuiteFitness.class);
    ...
    /**
     * <p>
     * Constructor for RRTSuiteFitness.
     * </p>
     */
    public RRTSuiteFitness() {
     ...
     * </p>
     */
    public RRTSuiteFitness(ClassLoader classLoader) {
...
//	}
    ...
    public double getFitness(
        ...
        logger.trace("Calculating RRT fitness");
        double fitness = 0.0;
        double bpBonus = 1.0;
        AbstractTestSuiteChromosome<? extends ExecutableChromosome> cSuite = (AbstractTestSuiteChromosome<? extends ExecutableChromosome>) suite.clone();
        AbstractTestSuiteChromosome<? extends ExecutableChromosome> cBuggySuite = (new OwnerChanger()).change((TestSuiteChromosome)suite.clone());
        List<ExecutionResult> originalResults = runTestSuite(
                cSuite
        );
        List<ExecutionResult> buggyResults = runTestSuite(
                cBuggySuite
        );
        List<TestResultZip> resultsZip = TestResultZip.create(originalResults,buggyResults);
        for (TestResultZip rz:
              resultsZip) {
            if(rz.isBugProvern()) fitness+=bpBonus;
        }
        return fitness;
